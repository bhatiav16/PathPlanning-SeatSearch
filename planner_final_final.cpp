...
vector<int> goal = {84,84,10}; //goal in example map for now
...
public:
  ...
  void Initialize(){
    ...
    string costChangeFileName = "Maps/Stadium_mod/allCostChanges_0PCT.csv";
...
double Planner::GetH(Node* state)
{
  ...
  return 0;//sqrt(pow(xDiff,2) + pow(yDiff,2) + pow(zDiff,2));
}
...
void Planner::Main()
{
  ...
  cout<<"[";
  string outFileName = "path0PCT.csv";
  ofstream outFile(outFileName);
  for(int i =0; i<costMap.path.size(); i++){
    auto pose = costMap.path[i];
    cout<< pose[0]+1 <<"," << pose[1]+1 << "," << pose[2]<< "," << " cost during traverse: " << pose[3];
...
int main()
{
  ...
  cout<<"accumulated cost: " << accumulatedCost << endl;
