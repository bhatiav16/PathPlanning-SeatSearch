...
struct Node{
  ...
  double g = 10000; //infinity
  double rhs = 10000; //infinity
  double f;
...
void Planner::ComputeShortestPath()
{
  while (PQ_Top(U)->key < CalculateKey(currState) || currState -> rhs != currState -> g)
  {
    ...
    std::cout << __FUNCTION__ << __LINE__ << std::endl;
    //cout<<(PQ_Top(U)->key < CalculateKey(currState));
...
void Planner::Main()
{
  ...
  while(currState->position != goalState->position)
  {
    ...
    // 6. Check if currState -> g = 10000, if so then no solution
    if (currState -> g == 10000)
    {
